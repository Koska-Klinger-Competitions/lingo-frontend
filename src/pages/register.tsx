...
const Register: NextPage = () => {
	...
	const query = useQuery<string[]>("langs", () => fetch(`${backendPath}/course/langs`	).then(res => res.json()));
	let langs: string[] = [];

	if (query.isLoading)
		return <Loading text="Loading language selection" />
	if (query.isSuccess)
		langs = query.data;
			...
			<div className="container flex grow flex-col items-center justify-center gap-20 px-4 py-16">
				...
				<section className="mx-auto grid w-full max-w-5xl grid-cols-1 flex-col gap-x-2 gap-y-3 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
					{languages.map((language) => (
						<Link
							key={language.name}
							href={langs.includes(language.code) ? "/learn" : "/gulag"}
							className={
								"max-h-40 flex cursor-pointer flex-col items-center gap-4 rounded-2xl border-2 border-b-4 border-gray-400 px-5 py-8 text-xl font-bold hover:bg-gray-300 hover:bg-opacity-20"
							}
