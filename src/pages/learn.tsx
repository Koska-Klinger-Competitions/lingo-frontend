...
function getTileColors ({
	...
	switch (status) {
		case "LOCKED":
			...
			return { borderColor: "#b7b7b7", background: "#e5e5e5" };
		case "COMPLETE":
			return { borderColor: "rgb(234, 179, 8)", background: "rgb(234, 179, 8)"};
		case "ACTIVE":
			return { borderColor, background };
...
function TileTooltip ({
	...
	const activebackground = unit?.background ?? "bg-green-500";
	const activeTextColor = unit?.textColor ?? "text-green-500";

	// Klingon Shens 
	let activityWorkaround = {};  
	if (status === "ACTIVE")
		activityWorkaround = { background: activebackground }
	...
	return (
		...
		>
			<div
				className={[
					"absolute z-30 flex w-[300px] flex-col gap-4 rounded-xl p-4 font-bold transition-all duration-300 yellow-500",
					status === "LOCKED"
							? "border-2 border-gray-200 bg-gray-100"
							: "bg-yellow-400",
					index === selectedTile ? "top-4 scale-100" : "-top-14 scale-0",
				].join(" ")}
				style={{ left: "calc(50% - 150px)", ...activityWorkaround}}
			>
				<div
					className={[
				...
				{status === "ACTIVE" ? (
					<Link
						href={`/lesson?lang=${language.code}&unit=${unitNumber}&lesson=${index+1}&level=${lessonsCompleted+1}`}
						className={[
									...
									case "fast-forward":
										...
										return (
											...
											>
												...
												/>
												<button
													className="absolute m-3 rounded-full border-b-8 p-4"
													style={getTileColors({
															tileType: tile.type,
															status,
...
function getTopBarColors(
	...
	// if (scrollY > units.length * steps) 
	// 	return defaultColors;  

	// return { background: units[scrollY / steps]!.background, border: units[scrollY / steps]!.border }; 
};
...
const Learn: NextPage = () => {
	...
	const query = useQuery<Unit[]>("units", () => createUnits(localStorage.getItem('current_course') ?? `${language.code}-beginner-basic`));

	if (query.isLoading) 
		return <Loading text="Loading your course..." />
	if (query.isSuccess)
		units = query.data;
	...
	return (
		<>
			<TopBar
				background={topBarColors.background}
				border={topBarColors.border}
			/>
...
const HoverLabel = ({
	...
	return (
		<div
			className={`absolute z-10 w-max animate-bounce rounded-lg border-2 border-gray-200 bg-white px-3 py-2 font-bold uppercase`}
			style={{
				top: "-25%",
		...
		>
			<header className="flex items-center justify-between gap-4 p-4">
				...
				<Link
					...
					className="flex items-center gap-3 rounded-2xl border-2 border-b-4 p-3 transition hover:text-gray-100"
